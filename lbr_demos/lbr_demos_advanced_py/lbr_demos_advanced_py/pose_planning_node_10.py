...
"""
socket assembly的探索版本。(拟完善探索完成自主完成轴孔装配的环节、主动优化、最后的建图演示)
    ...
    def As_planner(self):
        if self.initial is not None and self.target is not None:  ##满足轨迹规划的基本条件
            ...
            if self.N == 0:  ## 初始时机械臂向下运动直到接触孔平面
                ...
                self.get_logger().info("start the contour refincement phase.")
            ...
            if self.force_control_flag:  ##接触力调整与推理阶段，保持一个恒定的接触力
                ...
                if self.N == 4:
                    ...
                    np.savez(f'contact_slam_N4data_2511101625_{str(self.num)}.npz', particles=self.particles,
                             ...
                             Pose_tool_list=np.array(self.Pose_tool_pre, dtype=np.float64),
                             particles_filter=self.particles_filter,
                ...
                if self.N == 2:
                    ...
                    std = np.std(self.particles_filter, axis=0)
                    if std[0] < 0.9 and std[1] < 0.9:
                        self.N = 6  ## 当粒子的范围足够小时，进入mapping增强阶段
                        self.dz_flag = 0
                        self.get_logger().info('start the contour refincement phase!')
                        self.get_logger().info(f'The current particles distribution is {self.particles_filter}')
                    else:
                        ...
                        if self.N == 0:
                            ...
                            if std[0] < 0.9 and std[1] < 0.9:
                                ...
                                self.particles, self.weights = self.M.resample_particles(self.particles_filter,
                                                                                         ...
                                                                                         particles_remain)
                                ## 将粒子限制在狭义运动范围内
                                particle_range = [np.array([[self.M.x_min_socket, self.M.y_min_socket],
                                                            [self.M.x_max_socket, self.M.y_min_socket],
                                                            [self.M.x_max_socket, self.M.y_max_socket],
                                                            [self.M.x_min_socket, self.M.y_max_socket]])]
                                ...
                                if f_z < 0.9 and self.dz_flag == 2:  ## 落入孔区域（边缘可能有接触）
                                    if abs(f_y) > 1.0 or abs(f_x) > 0.9:  ##横向有约束
                                        ...
                                        if self.N_ite >= 2:
                                            ...
                                            self.Z_obs.append(self.z)
                                            self.Pose = np.append(self.Pose, current_pose[:2]).reshape(-1, 2)
                                            self.Pose_tool = np.append(self.Pose_tool, Pose_tool_curr[:2]).reshape(-1,
                                                                                                                   2)
                                            self.F_x_N.append(f_x), self.F_y_N.append(f_y), self.F_z_N.append(f_z)
                                            ## 重置以接收新目标（但先不接收初始位置）
                                            ...
                                            if self.i % 10 == 0:
                                                self.Z_obs.append(self.z)
                                                self.Pose = np.append(self.Pose, current_pose[:2]).reshape(-1, 2)
                                                self.Pose_tool = np.append(self.Pose_tool, Pose_tool_curr[:2]).reshape(
                                                    -1, 2)
                                                self.F_x_N.append(f_x), self.F_y_N.append(f_y), self.F_z_N.append(f_z)
                                else:  ## 竖向仍然接触
                                    self.z = 0
