...
"""
使用 Lazy RTDP-Bel 惰性动作评估进行主动探索（基于 ablation_test.py）
"""
...
class Motion_planning:
    ...
    def lazy_rtdp_bel_action(self, particles, weights, objects):
        ...
        ## Update Q-values(b)：只重新 backup 已经 evaluated 的动作。
        for action, action_key in zip(self.actions, action_keys):
...
if __name__ == "__main__":
    ...
    for p in range(22, 25):
        # 每次迭代的新目录
        folder = f"lazy_search_two_pin/ablation_control_{p}"  # 或用时间戳
        ...
        v_pred = M_planning.lazy_rtdp_bel_action(particles, weights, Movable_objects)
        while (
            ...
            # Lazy RTDP-Bel 按需评估当前 belief 上最有希望的动作
            peaks = M_planning.find_local_maximum(particles, weights, radius=5)
            print("len(peaks)=", len(peaks))
            v_pred = M_planning.lazy_rtdp_bel_action(
                particles_now, weights, Movable_objects
            )
