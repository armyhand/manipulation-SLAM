...
import numpy as np
from .motion_planning_5 import Motion_planning
...
"""
push block正式版，可以实现自主探索与规划（环境已知）
"""
...
class MinimumJerkPosePlanner(Node):
    def __init__(self):
        ...
        self.Movable_objects = [
            np.array([[-15, 15], [-15, -15], [15, -15], [15, 15]], dtype=float),  # 小矩形1
        ]
        # rotated = []
        ...
        ## 间距设置
        curve_dis = np.array([60.0, 60.0, 60.0, 60.0]) ##[delta_x_min,delta_x_max, delta_y_min, delta_y_max]
        self.M = Motion_planning(d=1.5, padding=curve_dis)  # 这里的单位是mm
        self.scene, self.xs, self.ys = self.M.discretize_scene(self.obstacles_all)

        self.hole_pose = np.array([-160.0, 180.0]) ##装配孔的位置
    ...
    def As_planner(self):
        if self.initial is not None and self.target is not None:  ##满足轨迹规划的基本条件
            ...
            qd = q0
            self.mu = 1.0
            self.v_move = 0.006  ##移动的速度（m/s）
            if self.N == 0:  ## 初始时机械臂向下运动直到接触作业平面
            ...
            if self.N == 1:  ##初始接触作业平面，需要进一步选取运动方向
                self.dz_flag = 0
                # 检测峰值，选择相应的方向
                ...
                if self.N_ite == 0:
                    ...
                    self.pose_ref, self.action_list, self.path_list = self.M.gain_information_optimize(
                    ...
                    self.get_logger().info(f"The current pin pose: {self.pose_ref}.")
                    self.v_pred = self.action_list[0]  ##先固定选择第一个动作
                    distance_thr = self.path_list[0][0]  ##选择第一个动作对应的距离作为阈值
                    print(f'Chosen velocity: {self.v_pred}, distance threshold: {distance_thr}')
                else:
                    self.v_pred = self.action_list[self.N_ite]
                    distance_thr = self.path_list[self.N_ite][0]
                    print(f'Continuing with next action: {self.v_pred}, distance threshold: {distance_thr}')

                # 将运动方向转化为运动距离
                delta = self.v_pred * distance_thr / 1000.0  ##这里的单位为m
                # 加入到轨迹规划中
                Points_recover.append(start + np.array([0.5 * delta[0], 0.5 * delta[1], 0.0]))
                Points_recover.append(start + np.array([delta[0], delta[1], 0.0]))
                ...
                self.get_logger().info(f"The current dz_flag={self.N_ite}")
    ...
    def publish_at_time(self):
        ...
        if all(v is not None for v in (self.Fr_r, self.Fr_l, self.Mr_r, self.Mr_l)):
            ...
            ##计算moving trend
            l_curr_l = (
                                   self.world_to_gripper_translation * self.gripper_to_sensor_translation * self.sensor_to_tool_translation_left * geo.V4(
                               0, 0, 0, 1))[0: 3]
            l_curr_r = (
                                   self.world_to_gripper_translation * self.gripper_to_sensor_translation * self.sensor_to_tool_translation_right * geo.V4(
                               0, 0, 0, 1))[0: 3]
            ...
            if self.force_control_flag:  ##接触力调整与推理阶段，保持一个恒定的接触力
                ...
                if self.N == 2:
                    ...
                    if std[0] < 1.2 and std[1] < 1.2:
                        ...
                        self.centers, self.clusters = self.M.extract_circle_clusters(particles_init, radius=1.0)
                        # 根据centers候选位置选择
                        self.pose_ref, self.action_list, self.path_list = self.M.gain_information_optimize(
                            Object=self.Movable_objects,
                            clusters=self.clusters,
                        ...
                        if max(np.max(self.M.Gain_information_optimize),
                            ...
                            if np.max(self.M.Gain_information_optimize) >= np.max(self.M.Gain_information):
                                print(
                                    f'The max entropy is {np.max(self.M.Gain_information_optimize)}, the chosen cluster is {self.pose_ref}')
                                # self.N = 5
                                self.N_ite = 0
                                self.dz_flag = 1
                            else:
                                ...
                                self.action_list = np.zeros(0)
                                self.path_list = np.zeros(0)
                                self.N_ite = 0  ##判定第几个action_list
                                self.dz_flag = 0
                        else:
                            print(
                ...
                if self.X is not None:
                    if self.n_now < self.X.shape[0]:  ##轨迹追踪还没有进行完
                        ...
                        if self.N == 0 and current_pose[2] < 0.256: ##限位
                            ## 收集当前的接触状态信息
                            self.z = 1
                        ...
                        elif np.linalg.norm(f_projec) > self.mu * f_ref and (f_vec @ self.v_vec) < 0 and np.linalg.norm(
                                f_projec) > 0.2 \
                                and f_ref > 1.0:
                            ...
                            self.X, self.Y, self.Z = None, None, None
                            self.force_inside_flag = False
                        elif self.N != 2 and (current_pose[0] > 0.755 or current_pose[0] < 0.46787 or current_pose[1] > 0.19959 or current_pose[1] < -0.26987): ##边界阈值，进一步完善
                            self.z = -1
                            self.Z_obs.append(self.z)
                            self.Pose = np.append(self.Pose, current_pose[:2]).reshape(-1, 2)
                            self.F_x_N.append(f_x), self.F_y_N.append(f_y), self.F_z_N.append(f_z)
                            ...
                            if self.N == 0:  ##处于竖直向下运动阶段
                                ...
                                if self.i % 10 == 0:
                                    ...
                                    self.Pose = np.append(self.Pose, current_pose[:2]).reshape(-1, 2)
                                    self.F_x_N.append(f_x), self.F_y_N.append(f_y), self.F_z_N.append(f_z)
                            elif self.N == 1 or self.N == 4 or self.N == 5:  ##处于水平运动阶段(探索或寻优)
                                self.z = 0
                                if self.i % 10 == 0:
                                    self.Z_obs.append(self.z)
                                    self.Pose = np.append(self.Pose, current_pose[:2]).reshape(-1, 2)
                        ...
                        if self.N == 5:## N5寻优模式，执行完当前轨迹
                            if self.N_ite >= self.action_list.shape[0]: ##
                                self.get_logger().info('finish the optimization trajectory!')
                                self.N = 1
                            self.force_control_flag = True  ##也进入接触力调整和推理阶段
                            self.total_time = 0.0
                            self.n_now = 0
                            ...
                            if self.N_ite >= self.action_list.shape[0]: ##
                                self.get_logger().info('Find the goal!')
                                self.N = 6
                            self.force_control_flag = True  ##也进入接触力调整和推理阶段
                            self.total_time = 0.0
                            self.n_now = 0
...
def main(args=None):
    ...
    try:
        ...
        np.savez(r'Force_data_2511101625.npz', Force_left=to_numpy_float(node.Force_left),
                 Force_right=to_numpy_float(node.Force_right),
                 Matrix_left=to_numpy_float(node.Matrix_left), Matrix_right=to_numpy_float(node.Matrix_right),
                 Force_dis_left=to_numpy_float(node.Force_dis_left),
