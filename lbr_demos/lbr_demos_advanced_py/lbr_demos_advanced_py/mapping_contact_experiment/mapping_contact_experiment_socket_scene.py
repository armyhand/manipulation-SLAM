...
注意：``OBSTACLE_TRUE`` 在这里只作为仿真 oracle 和误差评估使用。socket
场景中每个单独多边形保持与 ``OBSTACLE_REF`` 对应多边形相同的形状；一旦
观测到该多边形的一条边，就用该边支撑线反推该多边形的整体缩放。
"""
...
def _raw_signed_area(poly: np.ndarray) -> float:
    shifted = np.roll(poly, -1, axis=0)
    return 0.5 * float(np.sum(poly[:, 0] * shifted[:, 1] - poly[:, 1] * shifted[:, 0]))


def _simplify_socket_contour(
    poly: np.ndarray,
    min_edge_length: float = 1.2,
    parallel_angle_deg: float = 12.0,
) -> np.ndarray:
    """合并 socket 参考轮廓中的短边和近似共线边，降低探索复杂度。"""

    points = np.asarray(poly, dtype=float).copy()
    cos_tol = math.cos(math.radians(parallel_angle_deg))
    changed = True
    ...
    if _raw_signed_area(points) < 0.0:
        points = points[::-1]
    return points


pkl_path = Path(__file__).resolve().parent.parent / "socket_contours_2.pkl"
with open(pkl_path, "rb") as f:
    loaded = pickle.load(f)
OBSTACLE_TRUE_RAW = [np.asarray(poly, dtype=float) for poly in loaded]
OBSTACLE_TRUE = [poly.copy() for poly in OBSTACLE_TRUE_RAW]

# 参考轮廓：与真实轮廓完整等比例对应，不再合并短边或近似平行边。
OBSTACLE_REF = [0.7 * obs for obs in OBSTACLE_TRUE]
...
# Socket 场景的触觉状态不是“刚好零面积相切”，而是 pin 进入凹陷区域后
# 与某个 obstacle 有足够重合，同时接触边法线方向相互迎向。
SOCKET_MIN_OVERLAP_RATIO = 0.1
SOCKET_CANDIDATE_MIN_OVERLAP_RATIO = 0.02
SOCKET_FULL_OVERLAP_RATIO = 0.8
SOCKET_EDGE_DISTANCE_TOL = 0.5
SOCKET_NORMAL_DOT_TOL = 0.1
SOCKET_CONTACT_SEARCH_SAMPLES = 28
SOCKET_SCALE_LINE_TOL = 30.0
MIN_CONTOUR_SCALE = 0.05
MAX_CONTOUR_SCALE = 20.0
GLOBAL_VERTEX_RANGE_EDGE_LIMIT = 48
MAX_PROBE_EVALUATIONS_PER_STEP = 36
MAX_DEEP_PROBE_EVALUATIONS_PER_STEP = 72
MAX_PROBE_EVALUATIONS_PER_GROUP = 4
MAX_PATH_OPTIONS_PER_PROBE = 2
PROBE_LATERAL_SAMPLES = 3
MAX_TARGET_EDGES_PER_CONTOUR = 2
MAX_HYPOTHESIS_EDGES_PER_CONTOUR = 4
MAX_DIRECT_PLANS_TO_VALIDATE = 8
MAX_SWEEP_LANES_PER_AXIS = 3
MAX_SWEEP_PATHS_PER_STEP = 8
MAX_UNCERTAINTY_HYPOTHESES = 24
MAX_ACTIVE_HYPOTHESES = 1200
TARGET_MAX_VERTEX_ERROR_MM = 2.0
MAP_TRANSLATION_REGULARIZATION = 0.05
...
class ContactObservation:
    ...
    socket_edge_consts: dict[int, float] = field(default_factory=dict)
    message: str = ""
...
def group_support_lines(
    ...
    for edge in ref_edges:
        ...
        object_support = support_max_value(objects, edge.outward)
...
def fit_similarity_from_observed(
    ref_edges: list[Edge],
    observed: dict[int, float],
    min_rank: int = 3,
    max_residual_tol: float = SOCKET_SCALE_LINE_TOL,
) -> SimilarityFit | None:
    """由若干边支撑线观测拟合 ``world = scale * ref + translation``。

    对每条已观测边有线性约束：
    ``scale * ref_const + dot(translation, outward) = observed_const``。
    单次接触不足以确定三维未知量 ``(scale, tx, ty)``，只有观测法线组合达到
    足够秩时才返回拟合结果。
    """

    if len(observed) < min_rank:
        return None
    rows: list[list[float]] = []
    rhs: list[float] = []
    for edge_i, const in observed.items():
        edge = ref_edges[edge_i]
        rows.append([edge.const, float(edge.outward[0]), float(edge.outward[1])])
        rhs.append(float(const))
    a = np.asarray(rows, dtype=float)
    b = np.asarray(rhs, dtype=float)
    if np.linalg.matrix_rank(a, tol=1e-9) < min_rank:
        return None
    solution, *_ = np.linalg.lstsq(a, b, rcond=None)
    scale = float(solution[0])
    translation = np.asarray(solution[1:3], dtype=float)
    residuals = a @ solution - b
    max_residual = float(np.max(np.abs(residuals))) if len(residuals) else 0.0
    if not math.isfinite(scale) or not np.all(np.isfinite(translation)):
        return None
    if not (MIN_CONTOUR_SCALE <= scale <= MAX_CONTOUR_SCALE):
        return None
    if max_residual > max_residual_tol:
        return None
    return SimilarityFit(scale=scale, translation=translation, max_residual=max_residual)


def similarity_constants(ref_edges: list[Edge], fit: SimilarityFit) -> dict[int, float]:
    """把全局 similarity 拟合转换成所有边的支撑线常数。"""

    return {
        edge.index: float(fit.scale * edge.const + np.dot(fit.translation, edge.outward))
        for edge in ref_edges
    }
...
def groups_from_hypothesis(
    ...
    fit = fit_similarity_from_observed(ref_edges, hypothesis.observed)
    if fit is not None:
        for edge_i, const in similarity_constants(ref_edges, fit).items():
...
def contour_edge_indices(ref_edges: list[Edge]) -> dict[int, list[int]]:
    """按 contour 收集边索引，保持局部边序。"""

    by_contour: dict[int, list[Edge]] = {}
    for edge in ref_edges:
        by_contour.setdefault(edge.contour_index, []).append(edge)
    return {
        contour_i: [edge.index for edge in sorted(edges, key=lambda e: e.local_index)]
        for contour_i, edges in by_contour.items()
    }


def contour_vertex_mean(ref_edges: list[Edge], contour_index: int) -> np.ndarray:
    """返回参考 contour 的顶点均值，作为整体缩放中心。"""

    points = [
        edge.p0
        for edge in sorted(
            (edge for edge in ref_edges if edge.contour_index == contour_index),
            key=lambda e: e.local_index,
        )
    ]
    if not points:
        raise ValueError(f"Unknown contour index: {contour_index}")
    return np.mean(np.vstack(points), axis=0)


def contour_scale_center(ref_edges: list[Edge], contour_index: int) -> np.ndarray:
    """返回 contour 整体缩放中心。

    本场景只取消“真实缩放比例已知”的假设，仍沿用 obstacle_ref 与
    obstacle_true 共用坐标原点的整体缩放模型。因此单条边观测可以反推出
    该 contour 的整体缩放比例，但不会使用真实的 ref->true 比例。
    """

    return np.zeros(2, dtype=float)


def contour_scale_from_edge_const(
    ref_edges: list[Edge],
    edge_i: int,
    observed_const: float,
    center: np.ndarray | None = None,
) -> float | None:
    """由某条边支撑线常数反推该 contour 的整体缩放比例。

    缩放模型为 ``p_scaled = center + s * (p_ref - center)``。边方向保持不变，
    所以第 i 条边的支撑线常数满足
    ``const_i(s) = dot(center, outward_i) + s * (ref_const_i - dot(center, outward_i))``。
    """

    edge = ref_edges[edge_i]
    if center is None:
        center = contour_scale_center(ref_edges, edge.contour_index)
    center_const = float(np.dot(center, edge.outward))
    denom = edge.const - center_const
    if abs(denom) <= 1e-9:
        return None
    return float((observed_const - center_const) / denom)


def contour_scaled_consts(
    ref_edges: list[Edge],
    contour_index: int,
    scale: float,
    center: np.ndarray | None = None,
) -> dict[int, float]:
    """返回某个 contour 在整体缩放比例下的所有边支撑线常数。"""

    if center is None:
        center = contour_scale_center(ref_edges, contour_index)
    consts: dict[int, float] = {}
    for edge in ref_edges:
        if edge.contour_index != contour_index:
            continue
        center_const = float(np.dot(center, edge.outward))
        consts[edge.index] = center_const + scale * (edge.const - center_const)
    return consts


def all_contour_scaled_consts(
    ref_edges: list[Edge],
    scale: float,
) -> dict[int, float]:
    """返回全局缩放比例下所有 contour 的支撑线常数。"""

    consts: dict[int, float] = {}
    for contour_i in contour_edge_indices(ref_edges):
        consts.update(contour_scaled_consts(ref_edges, contour_i, scale))
    return consts


def contour_scaled_vertices(
    ref_edges: list[Edge],
    contour_index: int,
    scale: float,
    center: np.ndarray | None = None,
) -> np.ndarray:
    """返回某个 contour 在整体缩放假设下的顶点。"""

    if center is None:
        center = contour_scale_center(ref_edges, contour_index)
    points = [
        center + scale * (edge.p0 - center)
        for edge in sorted(
            (edge for edge in ref_edges if edge.contour_index == contour_index),
            key=lambda e: e.local_index,
        )
...
def scaled_edge_segment(
    ...
    center = contour_scale_center(ref_edges, edge.contour_index)
    return center + scale * (edge.p0 - center), center + scale * (edge.p1 - center)


def scaled_contour_observation_consts(
    ...
    """把一次边观测扩展成对应 contour 的整体缩放观测。"""

    by_contour: dict[int, list[int]] = {}
    for edge_i in candidate_edges:
        by_contour.setdefault(ref_edges[edge_i].contour_index, []).append(edge_i)

    inferred: dict[int, float] = {}
    for contour_i, edge_indices in by_contour.items():
        center = contour_scale_center(ref_edges, contour_i)
        scale_values: list[float] = []
        for edge_i in edge_indices:
            observed_const = observed_const_for_edge(obs, ref_edges[edge_i], objects)
            scale = contour_scale_from_edge_const(
                ref_edges,
                edge_i,
                observed_const,
                center=center,
        ...
        scale = float(np.mean(scale_values))
        contour_consts = all_contour_scaled_consts(ref_edges, scale)
        for edge_i in edge_indices:
            expected_const = contour_consts[edge_i]
            observed_const = observed_const_for_edge(obs, ref_edges[edge_i], objects)
            if abs(expected_const - observed_const) > max(line_tol, SOCKET_SCALE_LINE_TOL):
                return None
        inferred.update(contour_consts)
    return inferred
...
def hypothesis_is_consistent(
    ...
    除了每次观测局部可行外，还要求该假设下的整体现有轮廓是简单多边形、
    位于探索边界内，并且每个已分配接触点都落在对应边段的切向跨度内。
    """

    try:
        constants = hypothesis_constants(ref_edges, base_groups, hypothesis)
        poly = polygon_from_supports(ref_edges, constants)
    except (ValueError, np.linalg.LinAlgError):
        return False

    # 单条 socket 接触会先确定某条支撑线，而相邻边仍可能停留在先验位置。
    # 这种中间态可能暂时自交或让当前估计边段不覆盖真实触发点；这些不应删除
    # 正确观测，最终显示/误差评估仍由 constrained_estimated_polygon 保守回退。
    if not bool(np.all(np.isfinite(poly))):
        return False
    ...
    for step_i, edge_i in hypothesis.contact_pairs:
        ...
        if abs(observed_const - constants[edge_i]) > max(line_tol, SOCKET_SCALE_LINE_TOL):
            return False
    return True
...
def prune_active_hypotheses(hypotheses: list[Hypothesis], ref_edges: list[Edge]) -> list[Hypothesis]:
    """限制活动假设数量，优先保留约束秩更高、观测更多的分支。"""

    unique = prune_duplicate_hypotheses(hypotheses)
    if len(unique) <= MAX_ACTIVE_HYPOTHESES:
        return unique

    def score(hyp: Hypothesis) -> tuple[int, int, int]:
        observed_edges = set(hyp.observed)
        fit = fit_similarity_from_observed(ref_edges, hyp.observed)
        has_fit = 1 if fit is not None else 0
        return (
            has_fit,
            similarity_constraint_rank(ref_edges, observed_edges),
            len(observed_edges),
        )

    return sorted(
        unique,
        key=lambda hyp: (*score(hyp), -fit_residual_score(ref_edges, hyp)),
        reverse=True,
    )[:MAX_ACTIVE_HYPOTHESES]


def fit_residual_score(ref_edges: list[Edge], hyp: Hypothesis) -> float:
    """拟合残差代价；越小表示多次观测越支持同一个 similarity。"""

    fit = fit_similarity_from_observed(ref_edges, hyp.observed)
    if fit is None:
        return math.inf
    return fit.max_residual


def regularized_fit_score(ref_edges: list[Edge], hyp: Hypothesis) -> float:
    """对已拟合假设的温和先验代价，避免未知尺度/平移无约束漂移。"""

    fit = fit_similarity_from_observed(ref_edges, hyp.observed)
    if fit is None:
        return math.inf
    return float(
        abs(math.log(max(fit.scale, 1e-9)))
        + MAP_TRANSLATION_REGULARIZATION * np.linalg.norm(fit.translation)
    )


def best_regularized_fit_hypothesis(
    ref_edges: list[Edge],
    hypotheses: list[Hypothesis],
) -> Hypothesis | None:
    """选择一个无需真值的正则化 MAP 假设用于当前轮廓估计。"""

    fitted = [hyp for hyp in hypotheses if fit_similarity_from_observed(ref_edges, hyp.observed) is not None]
    if not fitted:
        return None
    return min(
        fitted,
        key=lambda hyp: (
            regularized_fit_score(ref_edges, hyp),
            fit_residual_score(ref_edges, hyp),
        ),
    )
...
def representative_candidate_edges(candidates: list[int], ref_edges: list[Edge]) -> list[int]:
    """为假设扩展选择每个 contour 的少量代表边。

    完整候选集合仍写入日志；这里仅限制搜索树分支数。优先保留当前 contour
    中参考边较长的边，通常能提供更稳定的支撑线/切向跨度约束。
    """
    ...
    for edge_i in candidates:
        by_contour.setdefault(ref_edges[edge_i].contour_index, []).append(edge_i)
    selected: list[int] = []
    for edge_indices in by_contour.values():
        selected.extend(
            sorted(
                edge_indices,
                key=lambda edge_i: -float(np.linalg.norm(ref_edges[edge_i].p1 - ref_edges[edge_i].p0)),
            )[:MAX_HYPOTHESIS_EDGES_PER_CONTOUR]
        )
...
def update_hypotheses_with_observation(
    ...
    for hyp in hypotheses:
        ...
        hypothesis_candidates = representative_candidate_edges(candidates, ref_edges)
        candidate_sets = [(edge_i,) for edge_i in hypothesis_candidates]

        for candidate_set in candidate_sets:
            next_consts = {
                edge_i: observed_const_for_edge(obs, ref_edges[edge_i], objects)
                for edge_i in candidate_set
            }
            valid = True
            for edge_i, const in next_consts.items():
                group = edge_group_map(hyp_groups)[edge_i]
                if not (group.min_const - line_tol <= const <= group.max_const + line_tol):
                    valid = False
                    break
                if edge_i in hyp.observed:
                    if abs(hyp.observed[edge_i] - const) > line_tol:
                        valid = False
                        break
                    const = hyp.observed[edge_i]
                next_consts[edge_i] = const
            if not valid:
                continue

            child = hyp.copy()
            for edge_i, const in next_consts.items():
                if edge_i in child.observed:
                    continue
                child.observed[edge_i] = const
            for edge_i in candidate_set:
                if (step_index, edge_i) not in child.contact_pairs:
                    child.contact_pairs.append((step_index, edge_i))
            fit = fit_similarity_from_observed(
                ref_edges,
                child.observed,
                max_residual_tol=max(line_tol, SOCKET_SCALE_LINE_TOL),
            )
            if fit is not None:
                fit_consts = similarity_constants(ref_edges, fit)
                fit_valid = True
                base_edge_to_group = edge_group_map(base_groups)
                for fit_edge_i, fit_const in fit_consts.items():
                    group = base_edge_to_group[fit_edge_i]
                    if not (group.min_const - line_tol <= fit_const <= group.max_const + line_tol):
                        fit_valid = False
                        break
                if not fit_valid:
                    continue
    ...
    return prune_active_hypotheses(expanded, ref_edges), sorted(all_candidates)


def sample_hypotheses_for_display(hypotheses: list[Hypothesis]) -> list[Hypothesis]:
    """为绘图/阶段性误差选择代表假设，避免大分支集拖慢探索。"""

    if len(hypotheses) <= MAX_UNCERTAINTY_HYPOTHESES:
        return hypotheses
    step = max(1, len(hypotheses) // MAX_UNCERTAINTY_HYPOTHESES)
    sampled = hypotheses[::step][:MAX_UNCERTAINTY_HYPOTHESES]
    if hypotheses[-1] not in sampled:
        sampled[-1] = hypotheses[-1]
    return sampled
...
def estimate_from_hypotheses(
    ...
    best_hyp = best_regularized_fit_hypothesis(ref_edges, hypotheses)
    if best_hyp is not None:
        return constrained_estimated_polygon(
            ref_edges,
            groups_from_hypothesis(base_groups, ref_edges, best_hyp),
        )
    sampled = sample_hypotheses_for_display(hypotheses)
    # 直接展开唯一一次使用的假设轮廓列表推导，减少中间包装函数。
    polygons = [
        constrained_estimated_polygon(
            ref_edges,
    ...
    vertices = [
        {
            ...
            "diameter_x": float(2.0 * radius_x),
            "diameter_y": float(2.0 * radius_y),
        }
...
def print_step_diagnostics(
    ...
    estimated_poly, _, _ = estimate_from_hypotheses(ref_edges, base_groups, hypotheses)
    ...
    for edge_i in record.candidate_edges:
        ...
        movable_pairs = movable_socket_edge_contact_pairs(
            objects,
            np.array(record.contact_ref, dtype=float),
            seg0,
            seg1,
            ref_edges[edge_i].outward,
            np.array(record.action, dtype=float),
        )
        if movable_pairs:
    ...
    for vertex_i, (edge, (center, radius_x, radius_y)) in enumerate(
        zip(
            ref_edges,
            vertex_uncertainty_ellipses_from_hypotheses(
                ref_edges,
                base_groups,
                hypotheses,
                boundary,
        ...
        x_min = float(center[0] - radius_x)
        x_max = float(center[0] + radius_x)
        y_min = float(center[1] - radius_y)
        y_max = float(center[1] + radius_y)
        print(
...
def support_max_value(objects: list[np.ndarray], direction: np.ndarray) -> float:
    """计算物体在某个方向上的最大投影，socket 内壁接触使用这一侧。"""

    return max(float(np.max(poly @ direction)) for poly in objects)
...
def socket_support_points_lateral_interval(
    objects: list[np.ndarray],
    ref_point: np.ndarray,
    normal: np.ndarray,
    tangent: np.ndarray,
    ...
    """计算 socket 接触侧（最大法向投影点）的切向范围。"""

    support = support_max_value(objects, normal)
    vals: list[float] = []
    for poly in objects:
        dots = poly @ normal
        support_pts = poly[np.isclose(dots, support, atol=tol)]
...
def movable_socket_edge_contact_pairs(
    objects: list[np.ndarray],
    ref_point: np.ndarray,
    obstacle_p0: np.ndarray,
    obstacle_p1: np.ndarray,
    obstacle_outward: np.ndarray,
    action: np.ndarray,
    tol: float = SOCKET_EDGE_DISTANCE_TOL,
) -> list[tuple[int, int]]:
    """返回满足 socket 法线条件的 movable-object/obstacle 边对。"""

    pairs: list[tuple[int, int]] = []
    obs_inner = -obstacle_outward
    if float(np.dot(action, obs_inner)) >= -SOCKET_NORMAL_DOT_TOL:
        return pairs
    for obj_i, poly in enumerate(objects):
        world_poly = poly + ref_point
        for edge_i, (p0, p1) in enumerate(zip(world_poly, np.roll(world_poly, -1, axis=0))):
            obj_out = object_edge_outward(p0, p1)
            if float(np.dot(action, obj_out)) <= SOCKET_NORMAL_DOT_TOL:
                continue
            if float(np.dot(obj_out, obs_inner)) >= -1e-9:
                continue
            if segment_distance(p0, p1, obstacle_p0, obstacle_p1) <= tol:
                pairs.append((obj_i, edge_i))
    return pairs


def movable_socket_normal_contact_pairs(
    objects: list[np.ndarray],
    obstacle_outward: np.ndarray,
    action: np.ndarray,
) -> list[tuple[int, int]]:
    """只按法线方向筛选 movable-object/socket 候选边对。

    在 obstacle 世界位置未知时，不能用某条参考边段的绝对端点做距离判定；
    但仍可要求 movable 边外法线顺着运动方向，且与 socket 内法线相向。
    """
    ...
    if float(np.dot(action, obs_inner)) >= -SOCKET_NORMAL_DOT_TOL:
        return pairs
    for obj_i, poly in enumerate(objects):
        for edge_i, (p0, p1) in enumerate(zip(poly, np.roll(poly, -1, axis=0))):
            obj_out = object_edge_outward(p0, p1)
            if float(np.dot(action, obj_out)) <= SOCKET_NORMAL_DOT_TOL:
                continue
            if float(np.dot(obj_out, obs_inner)) >= -1e-9:
...
def point_to_segment_distance(p: np.ndarray, a: np.ndarray, b: np.ndarray) -> float:
    """点到线段距离，用于复现 ``obs_extimation`` 的近接触边筛选。"""
...
def segment_distance(a0: np.ndarray, a1: np.ndarray, b0: np.ndarray, b1: np.ndarray) -> float:
    ...
    if segments_intersect(a0, a1, b0, b1, tol=1e-9):
        return 0.0
    return min(
...
def bounds_overlap(
    ...
    """判断两个 axis-aligned bounding boxes 是否相交。"""
...
def polygon_overlap_ratio(poly_a: np.ndarray, poly_b: np.ndarray) -> float:
    """计算 ``poly_a`` 被 ``poly_b`` 重合覆盖的面积比例。"""

    area_a = polygon_area(poly_a)
    if area_a <= 1e-12:
        return 0.0
    return polygon_intersection_area(poly_a, poly_b) / area_a


def object_edge_outward(p0: np.ndarray, p1: np.ndarray) -> np.ndarray:
    """CCW movable object 边的单位外法线。"""

    tangent = unit(p1 - p0)
    return np.array([tangent[1], -tangent[0]], dtype=float)


def socket_edge_overlap_candidates(
    action: np.ndarray,
    objects: list[np.ndarray],
    ref_point: np.ndarray,
    obstacle: np.ndarray | list[np.ndarray],
    obstacle_edges: list[Edge],
    min_overlap_ratio: float = SOCKET_MIN_OVERLAP_RATIO,
    ignored_edge_indices: set[int] | None = None,
) -> tuple[list[int], dict[int, float], float]:
    """按 socket 语义返回当前位置触发的 obstacle 边及其重合比例。

    判据对应 ``motion_planning_4.py::obs_extimation``：
    1. movable object 与 obstacle 有足够面积重合；
    2. 物体候选边外法线顺着运动方向，obstacle 候选边内法线逆着运动方向；
    3. 两条边相交或足够接近；
    4. 物体边外法线与 obstacle 边内法线夹角为钝角。
    """

    ignored_edge_indices = ignored_edge_indices or set()
    hit_edges: set[int] = set()
    edge_ratios: dict[int, float] = {}
    max_ratio = 0.0
    contours = as_contours(obstacle)
    contour_bounds = [polygon_bounds(contour) for contour in contours]
    for obj_i, obj_local in enumerate(objects):
        obj_world = obj_local + ref_point
        obj_area = polygon_area(obj_world)
        if obj_area <= 1e-12:
            continue
        obj_bounds = polygon_bounds(obj_world)
        contour_ratios = [
            (
                polygon_intersection_area(obj_world, contour) / obj_area
                if bounds_overlap(obj_bounds, contour_bound)
                else 0.0
            )
            for contour, contour_bound in zip(contours, contour_bounds)
        ]
        if not contour_ratios:
            continue
        max_ratio = max(max_ratio, max(contour_ratios))
        for obj_edge_i, (obj_p0, obj_p1) in enumerate(
            zip(obj_world, np.roll(obj_world, -1, axis=0))
        ):
            obj_out = object_edge_outward(obj_p0, obj_p1)
            if float(np.dot(action, obj_out)) <= SOCKET_NORMAL_DOT_TOL:
                continue
            for obs_edge in obstacle_edges:
                if obs_edge.index in ignored_edge_indices:
                    continue
                ratio = contour_ratios[obs_edge.contour_index]
                if ratio < min_overlap_ratio:
                    continue
                obs_inner = -obs_edge.outward
                if float(np.dot(action, obs_inner)) >= -SOCKET_NORMAL_DOT_TOL:
                    continue
                if float(np.dot(obj_out, obs_inner)) >= -1e-9:
                    continue
                if (
                    segment_distance(obj_p0, obj_p1, obs_edge.p0, obs_edge.p1)
                    > SOCKET_EDGE_DISTANCE_TOL
                ):
                    continue
                hit_edges.add(obs_edge.index)
                edge_ratios[obs_edge.index] = max(edge_ratios.get(obs_edge.index, 0.0), ratio)
    return sorted(hit_edges), edge_ratios, float(max_ratio)


def socket_penetration_compensation(
    objects: list[np.ndarray],
    edge: Edge,
    ratio: float,
) -> float:
    """返回 socket 触发位姿的支撑线补偿量。

    Socket 触发不是零面积相切，而是 movable object 已进入凹槽并达到一定
    重合比例。支撑线估计使用物体最大法向投影点，因此中等重合时会把障碍边
    推到真实边之外；这里用物体在该法线方向的投影宽度，把超过最小触发比例
    的面积重合近似成法向插入深度。

    当重合接近满覆盖时，触发点主要受边距离容差限制，直接按插入深度扣减会
    过补偿；此时只回补一半毫米的边距离容差。
    """

    normal_width = (
        support_max_value(objects, edge.outward)
        - support_min_value(objects, edge.outward)
    )
    if ratio >= SOCKET_FULL_OVERLAP_RATIO:
        return -SOCKET_EDGE_DISTANCE_TOL
    return max(0.0, (ratio - SOCKET_MIN_OVERLAP_RATIO) * normal_width)


def socket_observed_const_for_edge(
    obs: ContactObservation,
    edge: Edge,
    objects: list[np.ndarray],
) -> float:
    """根据 socket 触发观测估计某条候选边的支撑线常数。"""

    if obs.contact_ref is None:
        raise ValueError("Cannot compute support constant without contact_ref.")
    if edge.index in obs.socket_edge_consts:
        return obs.socket_edge_consts[edge.index]
    ratio = 0.0
    if obs.overlap_ratio > 0.0:
        ratio = max(SOCKET_MIN_OVERLAP_RATIO, min(SOCKET_FULL_OVERLAP_RATIO, obs.overlap_ratio))
    return float(
        np.dot(obs.contact_ref, edge.outward)
        + support_max_value(objects, edge.outward)
        - socket_penetration_compensation(objects, edge, ratio)
    )
...
def polygon_intersection_area(poly_a: np.ndarray, poly_b: np.ndarray) -> float:
    ...
    if not bounds_overlap(polygon_bounds(poly_a), polygon_bounds(poly_b)):
        return 0.0
...
def total_overlap_area(
    ...
    total = 0.0
    contour_bounds = [(contour, polygon_bounds(contour)) for contour in as_contours(obstacle)]
    for poly in objects:
        obj_world = poly + ref_point
        obj_bounds = polygon_bounds(obj_world)
        for contour, contour_bound in contour_bounds:
            if bounds_overlap(obj_bounds, contour_bound):
                total += polygon_intersection_area(obj_world, contour)
    return float(total)


def max_approach_distance_inside_boundary(
    contact_ref: np.ndarray,
    action: np.ndarray,
    objects: list[np.ndarray],
    boundary: ExplorationBoundary,
) -> float:
    """从预期接触位姿反向退让时，最多还能保持在探索边界内的距离。

    规划探测起点时会从 ``contact_ref`` 沿 ``-action`` 后退 ``approach_margin``。
    该函数逐顶点检查物体在后退过程中是否会越出已知探索边界，从而避免生成
    一个边界外的探测起点。
    """
    ...
    for poly in objects:
        for vertex in poly + contact_ref:
            for axis, lo, hi in (
...
def socket_contact_observation_at_ref(
    ref_point: np.ndarray,
    action: np.ndarray,
    ...
    """在给定参考点检查是否触发 socket 接触观测。"""

    hit_edges, edge_ratios, max_ratio = socket_edge_overlap_candidates(
        action,
        objects,
        ref_point,
        OBSTACLE_TRUE,
        true_edges,
        min_overlap_ratio=SOCKET_MIN_OVERLAP_RATIO,
        ignored_edge_indices=ignored_edge_indices,
    )
    if not hit_edges:
        return None

    overlap_area = total_overlap_area(objects, ref_point, OBSTACLE_TRUE)
    edge_consts: dict[int, float] = {}
    for edge_i in hit_edges:
        edge = true_edges[edge_i]
        edge_consts[edge_i] = float(
            np.dot(ref_point, edge.outward)
            + support_max_value(objects, edge.outward)
            - socket_penetration_compensation(objects, edge, edge_ratios.get(edge_i, max_ratio))
        )
    axis_const = float(np.mean([edge_consts[i] for i in hit_edges]))
    return ContactObservation(
        ok=True,
        action=action.copy(),
        start=np.asarray(segment_start, dtype=float).copy(),
        contact_ref=np.asarray(ref_point, dtype=float).copy(),
        distance=float(distance),
        plane_const=axis_const,
        axis_const=axis_const,
        overlap_area=overlap_area,
        overlap_ratio=max_ratio,
        max_path_overlap_area=0.0,
        true_edge_indices=hit_edges,
        candidate_edge_indices=list(hit_edges),
        socket_edge_consts=edge_consts,
    )
...
def first_contact_on_segment(
    ...
    Socket 场景中触觉状态改变要求已有一定面积重合。这里沿线段采样找到
    第一个触发区间，再二分定位触发位置。
    """
    ...
    previous_t = 0.0
    previous_hit = socket_contact_observation_at_ref(
        start,
        action,
        start,
        0.0,
        true_edges,
        objects,
        ignored_edge_indices,
    )
    if previous_hit is not None:
        return previous_hit

    for t in np.linspace(0.0, segment_distance, SOCKET_CONTACT_SEARCH_SAMPLES + 1)[1:]:
        ref = start + action * t
        hit = socket_contact_observation_at_ref(
            ref,
            action,
            start,
            float(t),
        ...
        for _ in range(16):
            mid = 0.5 * (low + high)
            mid_ref = start + action * mid
            mid_hit = socket_contact_observation_at_ref(
                mid_ref,
                action,
                start,
                mid,
...
def first_informative_contact_on_path(
    ...
    current_consensus = len(consensus_edges)
    already_observed_edges = set(consensus_edges)
    previously_explained_edges = {
        edge_i
        for previous_obs in observations
        for edge_i in previous_obs.candidate_edge_indices
    }

    for p0, p1 in zip(points[:-1], points[1:]):
        p0 = np.asarray(p0, dtype=float)
        p1 = np.asarray(p1, dtype=float)
        ignored_edges: set[int] = set(already_observed_edges) | set(previously_explained_edges)
        segment_start = p0
        while len(ignored_edges) <= len(true_edges):
            contact = first_contact_on_segment(
                segment_start,
                p1,
                true_edges,
                objects,
                ignored_edge_indices=ignored_edges,
            )
            if contact is None:
                traversed.append(p1)
                break
            ...
            before = len(ignored_edges)
            ignored_edges.update(contact.true_edge_indices)
            ignored_edges.update(candidate_edges)
            if len(ignored_edges) == before:
                return None, cleaned_axis_path(traversed), [], []
            if np.linalg.norm(contact.contact_ref - p1) <= 1e-8:
                break
            segment_start = contact.contact_ref.copy()
    return None, cleaned_axis_path(traversed), [], []
...
def similarity_constraint_rank(ref_edges: list[Edge], edge_indices: set[int]) -> int:
    """返回一组候选边对 similarity 参数 ``(scale, tx, ty)`` 的线性约束秩。"""

    if not edge_indices:
        return 0
    rows = [
        [
            ref_edges[edge_i].const,
            float(ref_edges[edge_i].outward[0]),
            float(ref_edges[edge_i].outward[1]),
        ]
        for edge_i in sorted(edge_indices)
    ]
    return int(np.linalg.matrix_rank(np.asarray(rows, dtype=float), tol=1e-9))


def edge_rank_gain_score(
    ref_edges: list[Edge],
    hypotheses: list[Hypothesis],
    edge_i: int,
) -> tuple[int, int, float]:
    """给规划候选边打分，优先补充能消除位姿/尺度歧义的独立约束。"""

    gains = 0
    post_ranks: list[int] = []
    edge = ref_edges[edge_i]
    for hyp in hypotheses:
        before_edges = set(hyp.observed)
        before_rank = similarity_constraint_rank(ref_edges, before_edges)
        after_rank = similarity_constraint_rank(ref_edges, before_edges | {edge_i})
        gains += max(0, after_rank - before_rank)
        post_ranks.append(after_rank)
    length = float(np.linalg.norm(edge.p1 - edge.p0))
    return gains, min(post_ranks, default=0), length


def sweep_line_paths(
    current_pos: np.ndarray,
    boundary: ExplorationBoundary,
    objects: list[np.ndarray],
) -> list[list[np.ndarray]]:
    ...
    half = object_half_extents(objects)
    x_min = boundary.x_min + half[0]
    x_max = boundary.x_max - half[0]
    y_min = boundary.y_min + half[1]
    y_max = boundary.y_max - half[1]
    if x_min > x_max or y_min > y_max:
        return []

    ys = np.linspace(y_min, y_max, MAX_SWEEP_LANES_PER_AXIS)
    xs = np.linspace(x_min, x_max, MAX_SWEEP_LANES_PER_AXIS)
    segment_paths: list[list[np.ndarray]] = []
    for y in ys:
        segment_paths.append([np.array([x_min, y], dtype=float), np.array([x_max, y], dtype=float)])
        segment_paths.append([np.array([x_max, y], dtype=float), np.array([x_min, y], dtype=float)])
    for x in xs:
        segment_paths.append([np.array([x, y_min], dtype=float), np.array([x, y_max], dtype=float)])
        segment_paths.append([np.array([x, y_max], dtype=float), np.array([x, y_min], dtype=float)])

    scored: list[tuple[float, list[np.ndarray]]] = []
    for segment in segment_paths:
        start = segment[0]
        transit_options = axis_aligned_path_candidates(current_pos, start, boundary, objects)
        if not transit_options:
            continue
        transit = min(transit_options, key=path_length_l1)
        path = cleaned_axis_path(transit + [segment[1]])
        if is_axis_aligned_path(path):
            scored.append((path_length_l1(path), path))
    unique: dict[tuple[tuple[float, float], ...], list[np.ndarray]] = {}
    for _, path in sorted(scored, key=lambda item: item[0]):
        signature = tuple((round(float(p[0]), 6), round(float(p[1]), 6)) for p in path)
        unique.setdefault(signature, path)
    return list(unique.values())[:MAX_SWEEP_PATHS_PER_STEP]


def first_fast_informative_contact_on_path(
    points: list[np.ndarray],
    true_edges: list[Edge],
    objects: list[np.ndarray],
    hypotheses: list[Hypothesis],
    ...
    for p0, p1 in zip(points[:-1], points[1:]):
        ...
        for tol_scale in (1.0, 2.0, 4.0):
            ...
            if not next_hypotheses:
                continue
            next_signatures = hypothesis_signature_set(next_hypotheses)
            next_consensus = len(consensus_observed_edges(next_hypotheses))
            if next_signatures != current_signatures or next_consensus > current_consensus:
                return contact, cleaned_axis_path(traversed), next_hypotheses, candidate_edges
        return None, cleaned_axis_path(traversed), [], []
...
def plan_next_sweep_probe(
    ...
    """用边界扫描寻找下一次能改变多假设集合的接触。"""

    current_signatures = hypothesis_signature_set(hypotheses)
    current_consensus_count = len(consensus_observed_edges(hypotheses))
    edge_to_group_key = {
        edge_i: key for key, group in base_groups.items() for edge_i in group.edge_indices
    }
    best: tuple[
    ...
    for path in sweep_line_paths(current_pos, boundary, objects):
        path_contact, full_path, next_hypotheses, candidate_edges = first_fast_informative_contact_on_path(
            path,
            true_edges,
            objects,
            hypotheses,
        ...
        if path_contact is None or not next_hypotheses:
            continue
        consensus_count = len(consensus_observed_edges(next_hypotheses))
        next_signatures = hypothesis_signature_set(next_hypotheses)
        if next_signatures == current_signatures and consensus_count <= current_consensus_count:
            continue
        rank_gain = max(
            (edge_rank_gain_score(ref_edges, hypotheses, edge_i)[0] for edge_i in candidate_edges),
            default=0,
        )
        target_edge = candidate_edges[0] if candidate_edges else path_contact.true_edge_indices[0]
        group_key = edge_to_group_key.get(target_edge, next(iter(base_groups)))
        probe = Probe(
            group_key=group_key,
            edge_index=target_edge,
            action=path_contact.action.copy(),
            start=path_contact.start.copy(),
            expected_contact=path_contact.contact_ref.copy(),
        )
        score = (
            len(next_hypotheses),
            -consensus_count,
...
def plan_next_disambiguating_probe(
    ...
    if observations:
        sweep_plan = plan_next_sweep_probe(
            current_pos,
            ref_edges,
            true_edges,
            objects,
    ...
    ranked_keys = sorted(
        ordered_keys,
        key=lambda key: (
            -edge_rank_gain_score(ref_edges, hypotheses, groups[key].edge_indices[0])[0],
            -edge_rank_gain_score(ref_edges, hypotheses, groups[key].edge_indices[0])[1],
            -edge_rank_gain_score(ref_edges, hypotheses, groups[key].edge_indices[0])[2],
        ),
    )

    contour_keys: dict[int, list[str]] = {}
    for key in ranked_keys:
        edge = ref_edges[groups[key].edge_indices[0]]
        contour_keys.setdefault(edge.contour_index, []).append(key)

    target_keys: list[str] = []
    for keys in contour_keys.values():
        target_keys.extend(
            sorted(
                keys,
                key=lambda key: (
            ...
            )[:MAX_TARGET_EDGES_PER_CONTOUR]
        )

    target_key_set = set(target_keys)
    fallback_keys = [key for key in ranked_keys if key not in target_key_set]
    ...
    for group_key in target_keys + fallback_keys:
        group = groups[group_key]
        probes = make_probe_variants(
            group,
            ref_edges,
            objects,
            predicted_const(group, groups),
        ...
        evaluated_for_group = 0
        for probe in probes:
            budget = (
                MAX_DEEP_PROBE_EVALUATIONS_PER_STEP
                if group_key not in target_key_set and best is None
                else MAX_PROBE_EVALUATIONS_PER_STEP
            )
            if evaluated_probes >= budget:
                break
            if evaluated_for_group >= MAX_PROBE_EVALUATIONS_PER_GROUP:
                break
            evaluated_probes += 1
            evaluated_for_group += 1
            ...
            direct_hypotheses: list[Hypothesis] = []
            direct_candidate_edges: list[int] = []
            for tol_scale in (1.0, 2.0, 4.0):
                direct_hypotheses, direct_candidate_edges = update_hypotheses_with_observation(
                    hypotheses,
                    planned_obs,
                    observations + [planned_obs],
                    ref_edges,
                    base_groups,
                    objects,
                    boundary,
                ...
                if direct_hypotheses:
                    break
            if not direct_hypotheses:
                continue

            consensus_count = len(consensus_observed_edges(direct_hypotheses))
            direct_signatures = hypothesis_signature_set(direct_hypotheses)
            is_noop = (
                direct_signatures == current_signatures
                and consensus_count <= current_consensus_count
            )
            if is_noop:
                continue
            direct_score = (
                len(direct_hypotheses),
                -consensus_count,
                0 if probe.edge_index in direct_candidate_edges else 1,
                len(direct_candidate_edges),
                float(np.sum(np.abs(planned_obs.start - current_pos)) + planned_obs.distance),
            )
            direct_plans.append(
                (
                    direct_score,
    ...
    for _, group_key, probe, planned_obs, _, _ in sorted(direct_plans, key=lambda item: item[0])[
        :MAX_DIRECT_PLANS_TO_VALIDATE
    ]:
        path_options = sorted(
            axis_aligned_path_candidates(current_pos, planned_obs.start, boundary, objects),
            key=path_length_l1,
        )[:MAX_PATH_OPTIONS_PER_PROBE]
        for transit_path in path_options:
            ...
            score = (
                len(next_hypotheses),
                -consensus_count,
                0 if probe.edge_index in candidate_edges else 1,
                len(candidate_edges),
                path_length_l1(full_path),
            )
            if best is None or score < best[0]:
            ...
            if probe.edge_index in candidate_edges and len(next_hypotheses) == 1:
                return group_key, probe, path_contact, full_path, next_hypotheses, candidate_edges
...
def action_matches_edge(action: np.ndarray, edge: Edge) -> bool:
    """判断四方向动作是否会触发 socket 内壁接触。

    对凹陷 socket，物体是沿 obstacle 外法线方向压入壁面；等价地，
    与 obstacle 内法线方向夹角为钝角。
    """

    return float(np.dot(action, edge.outward)) > SOCKET_NORMAL_DOT_TOL
...
def make_probe(
    ...
    boundary: ExplorationBoundary | None = None,
    lateral_value: float | None = None,
) -> Probe:
    """根据当前估计支撑线构造一次计划探测。

    ``predicted_const`` 是该边目前认为最可能的支撑线坐标。函数先计算预期接触
    参考点，再沿接触动作反方向退让一定距离作为探测起点；若给定探索边界，
    退让距离会被限制到物体仍在边界内。
    """
    ...
    contact_dot = predicted_const - support_max_value(objects, edge.outward)
    if lateral_value is None:
        lateral_value = float(np.dot(ref_mid, edge.tangent))
    expected_contact = edge.outward * contact_dot + edge.tangent * float(lateral_value)
    if boundary is not None:
        approach_margin = min(
            approach_margin,
            max_approach_distance_inside_boundary(
                expected_contact,
                action,
                objects,
                boundary,
...
def make_probe_variants(
    ...
    """沿当前可行边段跨度生成多个 socket 探针。

    凹陷 socket 的有效接触常常只发生在短边的一小段上；只用参考边中点会
    错过真实边段。这里用相邻支撑线给出的切向跨度采样，必要时以少量探针
    数换取可达性。
    """

    edge = ref_edges[group.edge_indices[0]]
    action = max(ACTIONS, key=lambda a: float(np.dot(a, edge.outward))).copy()
    ref_mid_lat = float(np.dot(0.5 * (edge.p0 + edge.p1), edge.tangent))
    lat_values = [ref_mid_lat]
    if groups is not None:
        lat_min, lat_max = edge_lateral_span_interval(
            edge.index,
            ref_edges,
            groups,
            action,
        ...
        if lat_min <= lat_max:
            lat_values.extend(np.linspace(lat_min, lat_max, PROBE_LATERAL_SAMPLES).tolist())
    else:
        lat_values.extend(
            np.linspace(edge.span[0], edge.span[1], min(PROBE_LATERAL_SAMPLES, 5)).tolist()
        )

    for lat in (edge.span[0], edge.span[1], 0.5 * (edge.span[0] + edge.span[1])):
        lat_values.append(float(lat))

    probes: list[Probe] = []
    seen: set[tuple[float, float, float]] = set()
    for margin in (approach_margin, 0.65 * approach_margin, 1.35 * approach_margin):
        for lat in lat_values:
            probe = make_probe(
                ...
                objects,
                predicted_const,
                approach_margin=float(margin),
                groups=groups,
                boundary=boundary,
                lateral_value=float(lat),
            )
            signature = (
                round(float(np.dot(probe.expected_contact, edge.outward)), 4),
                round(float(np.dot(probe.expected_contact, edge.tangent)), 4),
                round(float(np.linalg.norm(probe.start - probe.expected_contact)), 4),
            )
...
def simulate_contact(
    ...
    action = probe.action
    start = probe.start
    end = start + action * max_distance
    contact = first_contact_on_segment(start, end, true_edges, objects)
    if contact is None:
        return ContactObservation(
            ok=False,
            action=action.copy(),
            start=start.copy(),
            message=f"probe from edge {probe.edge_index} did not reach a socket contact",
        )
    return contact
...
def observed_const_for_edge(
    ...
    return socket_observed_const_for_edge(obs, edge, objects)
...
def candidate_edges_for_observation(
    ...
    for edge in ref_edges:
        ...
        obj_interval = socket_support_points_lateral_interval(
            objects,
            obs.contact_ref,
            edge.outward,
            edge.tangent,
        )
        if not intervals_overlap(obj_interval, span_guess, tol=span_pad):
            continue
        if not movable_socket_normal_contact_pairs(
            objects,
            edge.outward,
            obs.action,
        ):
...
def global_support_linear_constraints(
    ...
    for edge in ref_edges:
        ...
        local_support = socket_support_points_lateral_interval(
            objects,
    ...
    for coeff in vertex_coeffs:
        ...
        for dim in range(2):
            ...
            if not min_result.success or not max_result.success:
                return None
            values.append((float(min_result.fun), float(-max_result.fun)))
...
def vertex_uncertainty_ellipses(
    ...
    if boundary is not None and len(ref_edges) <= GLOBAL_VERTEX_RANGE_EDGE_LIMIT:
...
def run_strategy(
    ...
    prior_poly = [poly.copy() for poly in OBSTACLE_REF] ## 使用参考轮廓形状；真实缩放比例由接触观测反推
    ...
    while remaining:
        ...
        edge_to_group_key = {
            edge_i: key for key, group in groups.items() for edge_i in group.edge_indices
        }
        observed_group_key = edge_to_group_key.get(
            obs.candidate_edge_indices[0] if obs.candidate_edge_indices else probe.edge_index,
            group_key,
        )
        ...
        # Socket 接触允许一定面积重叠；这里仅记录路径上的最大重叠面积，
        # 真正的状态改变由 first_contact_on_segment 的重合比例+法线条件判定。
    ...
    strict_success = (
        success
        and line_group_observed_count(groups) == len(groups)
        and len(hypotheses) == 1
    )
    error_success = max_err <= TARGET_MAX_VERTEX_ERROR_MM
    if error_success and not strict_success:
        failure_reason = ""

    summary = {
        "strategy": strategy,
        "ordering": ordering,
        "success": strict_success or error_success,
        "failure_reason": failure_reason,
        "strict_contact_pair_success": strict_success,
        "error_threshold_success": error_success,
        "target_max_vertex_error_mm": TARGET_MAX_VERTEX_ERROR_MM,
        ...
        "max_contact_overlap_ratio": max(
            (record.overlap_ratio for record in records),
            default=0.0,
        ),
...
def main() -> None:
    ...
    parser.add_argument("--line-tol", type=float, default=2.0)
    parser.add_argument(
        "--strategies",
        nargs="+",
        choices=["reference_order", "axis_anchor_order", "nearest_next", "information_gain"],
        default=["reference_order", "axis_anchor_order", "nearest_next", "information_gain"],
